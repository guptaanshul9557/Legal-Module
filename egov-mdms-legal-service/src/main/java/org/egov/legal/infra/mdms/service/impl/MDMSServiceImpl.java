...
public class MDMSServiceImpl {
	...
	private static final Logger log = LoggerFactory.getLogger(MDMSServiceImpl.class);
	...
	public void run() {
		try {
			...
			System.out.println("List OF files which have Error while parsing " + errorFilesList);
			...
			throw e;  // Throw the custom exception
		} catch (Exception e) {
			log.error("Error occurred while loading files: {}", e.getMessage());
			throw new RuntimeException("General error during file loading", e);  // Throw runtime exception for any other errors
		}
	}
		...
		if (listOfFiles != null) {
			for (File file : listOfFiles) {
				if (file.isFile()) {
					...
					if (fileExtension.equalsIgnoreCase("json") || fileExtension.equalsIgnoreCase("yaml") || fileExtension.equalsIgnoreCase("yml")) {
						...
						try {
							...
							throw new JSONPARSEException("Error while parsing file: " + file.getAbsolutePath(), e);
						} catch (Exception e) {
							log.error("Error occurred while reading the file: {}", file.getAbsolutePath());
							errorList.add(file.getAbsolutePath());
							throw new FileReadingException("Error reading file: " + file.getAbsolutePath(), e);
						}
					}
	...
	public void prepareTenantMap(Map<String, Object> map) {
		...
		while (masterKeyIterator.hasNext()) {
			...
			try {
				...
				throw new JSONPARSEException("Error while parsing file!", e);
			}
			...
			if (!tenantMap.containsKey(tenantId)) {
				...
				if (!tenantModule.containsKey(moduleName)) {
					...
					boolean isMergeAllowed = isMergeAllowedForMaster(moduleName, masterName);
	...
	public boolean isMergeAllowedForMaster(String moduleName, String masterName) {
		...
		if (masterConfigMap.containsKey(moduleName)
			...
			try {
				isMergeAllowed = JsonPath.read(objectMapper.writeValueAsString(masterData), MDMSConstants.MERGE_FILES);
